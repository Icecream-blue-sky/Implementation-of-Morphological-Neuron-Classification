...
def _plot_3D_figure(SWC,my_color_list = ['r','g','b','c','m','y','r--','b--','g--']):
    ...
    SWC: dict
    """
    for index in list(SWC.keys()) : # not ordered but that has little importance here
...
def plot_2D_SWC(file_name='P20-DEV139.CNG.swc',cs=None,\
                ...
                show_axis=False,depth="Y",filter=list(range(10)),\
    ...
    if cs == None :
        ...
        Z = [[0,0],[0,0]]
        levels=list(range(int(np.min(cs)),int(np.max(cs)),100))
...
def plot_3D_SWC(file_name='P20-DEV139.CNG.swc',cs=None,synapses=None,syn_cs=None,outN=None,offset=None,align=True,filter=list(range(10))) :
    """
