...
class BTStats(object):
    ...
    #todo:再次标记，貌似是将子树根据指定level写入文件的方法。decompose by depth
    def write_subtree_by_level(self, level, path, neuronName):
        sub_tree_name = path + "/" + str(level-1) + "/"
        # sub_tree_name="./"+path+"/"
        if not os.path.exists(sub_tree_name):
            os.makedirs(sub_tree_name)
        sub_tree = self.get_tree_by_level(level)
        sub_tree.other_write_SWC_tree_to_file(str(sub_tree_name) + neuronName + "_" + str(level-1) + '.swc')
        return sub_tree

    #todo:标记标记！decompose by height
    #用以下思路将其转化为decompose by height：
    #已知每个neuron的depth：neuron_depth，计算出选定height_level对应的depth_level，则要分割depth_level到neuron_depth的子树，令from_level=当前depth_level对应的node_level(=depth_level)，to_level等于子节点的node_level即可。
    def write_subtree_by_range(self, from_level, to_level, path, neuronName, height_level):
        """
        This method gets slices from an specified level from_level to the terminals of the neuron.
        :rtype: subtree
        """
        new_file_name = str(height_level)
        sub_tree_name = path + "/" + new_file_name + "/"
        if not os.path.exists(sub_tree_name):
            os.makedirs(sub_tree_name)
        ...
        if (len(sub_tree.get_nodes()) > 1):
            sub_tree.other_write_SWC_tree_to_file(
                str(sub_tree_name)  + neuronName + '_' + new_file_name + '.swc')
        return sub_tree

    #todo:标记！decompose by slice
    #todo:就是论文中slice的意思，比如from_level = 0,to_level = 1，代表只保留depth为0的那层树。
    #从node的角度来解释就是：保存node_level=0到node_level=1的子树
    def write_subtree_by_slice_range(self, slice_begin, slice_end, path, neuronName):
        """
        ...
        if (slice_begin < slice_end and 0 <= slice_begin):
            #slice_depth and slice_size
            new_file_name = str(slice_begin) + "_" + str(slice_end-slice_begin)
            ...
            if len(sub_tree.get_nodes()) > 1:
                sub_tree.other_write_SWC_tree_to_file(str(sub_tree_name) + neuronName + '_' +
                                                      new_file_name + '.swc')
            else:
                sub_tree = None
            return sub_tree
    ...
    #todo:标记，获取指定level的subtree的一种方法，若level=1代表保留node_level为<=1的节点
    #会不会这里get_tree_by_level直接把tree的各个node都改变了？？？所有分割一次后就永远不变了？？？
    ...
    #todo:就是论文中slice的意思，比如from_level = 0,to_level = 1，代表只保留depth为0的那层树。
    #从node的角度来解释就是：保存node_level=0到node_level=1的子树，并且保留根节点，和根节点的soma_children
    #如果to_level超过neuron_depth+1，这个方法会默认分割到最大depth,即保留from_level之后的所有分支。
    def get_tree_by_range(self, from_level, to_level):
        ...
        for node in self._tree:
            if node != self._tree.root:
                if node.level == from_level and node.get_parent().level == from_level - 1:  # the initial level
                    ...
                    #没看懂这句，为什么要设定from_level处的节点的parent为树的根节点？？？就是为了保留soma作为节点。
                    node.set_parent(self._tree.root)
                    new_top_level.append(node)
        ...
        #from_level=1的情况不要特殊考虑吗？实验证明需要特殊考虑，否则会重复添加soma_children两次
        if (from_level == 0 or from_level == 1):  # case from_level==0 do not add anything just consider the current children on node
            soma_children = []
        else:
    ...
    def get_tree_by_range_new(self, from_level, to_level):
        ...
        soma_children = self.get_soma_children()#获取是soma类型的节点
        new_children = soma_children + new_top_level  # join old soma children and new soma children
        self._tree.root.children = []
        self._tree.root.children = new_children
    ...
    def fractal_dimension_box_counting_core(self, vg):
        ...
        szs = [2 ** x / float(max(vg.res)) for x in range(1, int(math.log(startDim, 2)) + 1)]
    ...
    def frac_dim_lac(self, vg=None):
        ...
        for i in range(1, len(bc.countVals)):
            ...
            bc.coverageVals[i] = s
        szs = [2 ** (3 * x) for x in range(1, int(math.log(startDim, 2)) + 1)]
        szs2 = [2 ** x / float(max(vg.res)) for x in range(1, int(math.log(startDim, 2)) + 1)]
        cover = bc.coverageVals[1:-1]
